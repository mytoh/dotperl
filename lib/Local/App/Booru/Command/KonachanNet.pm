...
package Local::App::Booru::Command::KonachanNet;
...
my sub get_posts :ReturnType(Maybe[HashRef]) ($ua, $page, $tags ) {
  ...
  my $url = URI->new("https://konachan.net/post.json");
  $url->query_form(
    tags  => $formatted_tags,
    limit => $limit,
    page  => $page
...
my sub download_post ( $ua, $post ) {
  ...
  if ( defined $post->{'file_url'} ) {
    my $url = 'https:' . $post->{'file_url'};
    my $output_file =
      $post->{'id'} . '-' . basename($url);
    if ( !-f $output_file ) {
      $ua->get($url)
        ->result
        ->content
...
sub description { "Get images from konachan.net" }
