...
package Local::App::Chan::Command::Lainchan;
...
my sub parse_images :ReturnType(ArrayRef[File]) ( $board, $json ) {
  ...
  my @images = map {
    my $url = URI->new("https://lainchan.org");
    my $filename = $_->{'tim'} . $_->{'ext'};
    $url->path_segments( decode_utf8($board), 'src', $filename);
    +{ filename => $filename,
       url => $url }
  }
...
my sub get_single ( $ua, $board, $thread ) {
  ...
  if (defined $thread_data
    ...
    if ( $images->@* ) {
      say 'Downloading ' . decode_utf8($board) . ': ' . $thread;
...
sub description { "lainchan script" }
