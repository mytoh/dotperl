...
my sub fetch_b_thread ( $obj, $server ) {
...
my sub find_b_server ($obj) {
    $obj->{'board'} = 'b';
    first { fetch_b_thread( $obj, $_ ) } $BOARD_SERVERS->{b}->@*;
}

my sub scrape_image_list ($obj) {
    my ( $mech, $server, $board, $thread ) =
      $obj->@{qw<mech server board thread>};
    my $url = "https://${server}.2chan.net/${board}/res/${thread}.htm";
    $mech->get($url);
    if ( $mech->success ) {
        my @uris =
